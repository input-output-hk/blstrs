...
// Generates coefficients for the given ranges and
// bit lenghts.
fn setup<C: CurveAffine>() -> Vec<C::ScalarExt> {
...
#[test]
fn ntt_correctness() {
    let coeffs= setup::<blstrs::G1Affine>();

    let mut coeffs_copy1 = coeffs.clone();
    let mut coeffs_copy2 = coeffs.clone();

    for (b_index, b) in BITS.iter().enumerate() {
        for k in MULTICORE_RANGE {
            println!("Testing gpu_{b}b_{k}...");

            blstrs::ntt_gpu(DEFAULT_GPU, &mut coeffs_copy1, NTTInputOutputOrder::NN);

            blstrs::ntt_gpu(DEFAULT_GPU, &mut coeffs_copy2, NTTInputOutputOrder::RR);
            assert!(coeffs_copy1 == coeffs_copy2);


            blstrs::intt_gpu(DEFAULT_GPU, &mut coeffs_copy1, NTTInputOutputOrder::NN);

            blstrs::intt_gpu(DEFAULT_GPU, &mut coeffs_copy2, NTTInputOutputOrder::RR);
            assert!(coeffs == coeffs_copy1);
            assert!(coeffs_copy1 == coeffs_copy2);

            blstrs::ntt_gpu(DEFAULT_GPU, &mut coeffs_copy1, NTTInputOutputOrder::NR);

            blstrs::intt_gpu(DEFAULT_GPU, &mut coeffs_copy1, NTTInputOutputOrder::RN);
            assert!(coeffs == coeffs_copy1);
        }
    }
